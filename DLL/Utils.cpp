...
void Utils::LOG(const char* txt) // log to both console and file
{

	//printf(txt);

	FILE* pFile = fopen("C:\\opse\\log.txt", "a");
	if (pFile == NULL)
	{
		//printf("Could not open file");
		MessageBoxA(0, "log.txt", "Error: log.txt couldn't be opened.", MB_OK);
		return;
	}
	fprintf(pFile, txt);
	fflush(pFile);
