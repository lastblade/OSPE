...
ADDRESS_VALUE Process::SearchMemory(void* p_pvStartAddress, DWORD p_dwSize, void* p_pvBuffer, DWORD p_dwBufferSize)
{
	...
	Utils::errorLog("[ERROR] SearchMemory did not find the pattern!");
...
ADDRESS_VALUE Process::SearchSignature(void* p_pvStartAddress, DWORD p_dwSize, void* p_pvBuffer, DWORD p_dwBufferSize)
{
	...
	Utils::errorLog("[ERROR] SearchSignature did not find the signature!");
...
SECTION_INFO Process::GetModuleSection(string p_sModule, string p_sSection)
{
	...
	if (!p_sModule.empty())
	{
		...
		for (size_t i = 0; i < vModules.size(); i++)
		{
			if (p_sModule.compare(Utils::ToLower(vModules[i].szModule)) == 0)
			{
				...
				if (hModule == NULL)
				{
					//errorLog("[ERROR] Cannot find module handle: ", p_sModule);
					return oSectionData;
				}
			}
		}
	...
	else
	{
		...
		if (hModule == NULL)
		{
			//errorLog("[ERROR] Cannot find default module handle: ", p_sModule);
			return oSectionData;
		}
	}
	...
	{
		Utils::errorLog("[ERROR] Cannot allocate space for sections: ", ntHeaders.FileHeader.NumberOfSections);
		return oSectionData;
	}
	...
	//errorLog("[ERROR] GetModuleSection did not find the section: ", p_sSection);
...
vector<MODULEENTRY32> Process::GetProcessModules(DWORD p_dwID)
{
	...
	{
		Utils::errorLog("[ERROR] Cannot get modules snapshot!");
		return vModules;
	}
	...
	{
		Utils::errorLog("[ERROR] Cannot get first module!");
		return vModules;
	}
